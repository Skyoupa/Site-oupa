...
// ================================
// ROUTE DEBUG ADMIN (À SUPPRIMER EN PRODUCTION)
// ================================

// GET /debug/admin-check
$router->get('/debug/admin-check', function() {
    global $db;
    
    try {
        // Vérifier les admins existants
        $admins = $db->fetchAll("SELECT id, username, email, role, status FROM users WHERE role = 'admin'");
        
        // Tester le hash du mot de passe admin
        $adminUser = $db->fetch("SELECT * FROM users WHERE email = 'admin@oupafamilly.com'");
        $passwordTest = null;
        
        if ($adminUser) {
            $passwordTest = Auth::verifyPassword('Oupafamilly2024!', $adminUser['hashed_password']);
        }
        ...
        ApiResponse::error('Debug error: ' . $e->getMessage(), 500);
    }
});

// POST /debug/recreate-admin (À SUPPRIMER EN PRODUCTION)
$router->post('/debug/recreate-admin', function() {
    global $db;
    ...
    try {
        ...
        // Recréer l'admin
        $result = Auth::createDefaultAdmin();
        
        if ($result) {
            ApiResponse::success(['message' => 'Admin recréé avec succès']);
        } else {
            ApiResponse::error('Erreur lors de la recréation admin', 500);
        }
        
    } catch (Exception $e) {
        ApiResponse::error('Erreur recréation: ' . $e->getMessage(), 500);
    }
});
