...
class Auth {
     ...
     * Compatible OVH Pack Starter
     */
    public static function hashPassword($password) {
        // Utiliser PASSWORD_DEFAULT pour la compatibilité OVH
        // Si ARGON2ID n'est pas disponible, utilise BCRYPT automatiquement
        if (defined('PASSWORD_ARGON2ID') && function_exists('password_hash')) {
            try {
                ...
                // Fallback vers BCRYPT si ARGON2ID échoue
                return password_hash($password, PASSWORD_BCRYPT, ['cost' => 12]);
            }
        } else {
            // Fallback pour OVH Pack Starter
            return password_hash($password, PASSWORD_BCRYPT, ['cost' => 12]);
        }
    }
